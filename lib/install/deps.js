...
function isDep (tree, child, cb) {
  ...
  try {
    ...
    return cb(child.fromShrinkwrap)
  }
  var matches
  ...
  asyncMap(args, function (pkg, done) {
    ...
    resolveWithNewModule(pkg, tree, log.newGroup('loadRequestedDeps'), iferr(depLoaded, function (child, tracker) {
      ...
      if (saveToDependencies) {
        tree.package[saveToDependencies][childName] = child.package._requested.saveSpec
      }
      if (saveToDependencies === 'optionalDependencies') {
        tree.package.dependencies[childName] = child.package._requested.saveSpec
      }
