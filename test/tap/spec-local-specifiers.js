...
test('specifiers', function (t) {
  ...
  slashes.forEach(function (os) {
    t.test('win-abs-drive-' + os, function (t) {
      common.npm(['install', '--dry-run', '--json', 'file:win-abs-drive-' + os], conf, function (err, code, stdout) {
        if (err) throw err
        t.not(code, 0, 'command errored ok')
        t.comment(stdout.trim())
        t.test('verify failure of file-not-found or wrong drive letter on windows')
        t.test('verify failure due to windows paths not supported on non-Windows')
        t.done()
      })
    })
  })
...
test('ambiguity', function (t) {
  ...
  t.test('arg: looks like package name, is dir', function (t) {
    common.npm(['install', 'mkdirp', '--dry-run', '--json'], conf, function (err, code, stdout) {
      ...
      const version = deps.mkdirp && deps.mkdirp.version
      t.is(version, '9.9.9', 'got local dir')
      t.done()
    })
  })
  t.test('arg: looks like package name, is package', function (t) {
    common.npm(['install', 'wordwrap', '--dry-run', '--json'], conf, function (err, code, stdout) {
      ...
      const result = JSON.parse(stdout.trim())
      const deps = result.dependencies || {}
      const version = deps.wordwrap && deps.wordwrap.version
      t.is(version, '0.0.2', 'got local dir')
      t.done()
    })
  })
  t.test('arg: looks like github repo, is dir', function (t) {
    common.npm(['install', 'example/minimist', '--dry-run', '--json'], conf, function (err, code, stdout) {
      ...
      const result = JSON.parse(stdout.trim())
      const deps = result.dependencies || {}
      const version = deps.minimist && deps.minimist.version
      t.is(version, '9.9.9', 'got local dir')
      t.done()
    })
  })
  t.test('package: looks like tag, has dir, use tag', function (t) {
    common.npm(['install', '--dry-run', '--json'], {cwd: path.join(testdir, 'prefer-pkg'), env: conf.env}, function (err, code, stdout) {
      ...
      const deps = result.dependencies || {}
      const version = deps.mkdirp && deps.mkdirp.version
      t.is(version, '1.0.0', 'got local dir')
      t.done()
    })
  })
...
test('install behavior', function (t) {
  ...
  t.test('transitive deps for in-larger-module cases', function (t) {
    common.npm(['install', 'file:ib-in'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
  ...
  t.test('transitive deps for out-of-larger module cases', function (t) {
    common.npm(['install', 'file:../ib-out'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
  ...
  t.test('tar/tgz/tar.gz should install a tarball', function (t) {
    common.npm(['install', 'file:tarball-1.0.0.tgz'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
  ...
  t.test('non tar/tgz/tar.gz files should give good error message', function (t) {
    common.npm(['install', 'file:noext'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
  ...
  t.test('directories without package.json should give good error message', function (t) {
    common.npm(['install', 'file:not-module'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.not(code, 0, 'error on dir w/o module')
      if (stdout) t.comment(stdout.trim())
      t.test('verify error message is not trash')
      t.done()
    })
  })
  t.test('verify preinstall step runs after finalize, such that cwd is as expected', function (t) {
    common.npm(['install', 'file:test-preinstall'], {cwd: ibdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
...
test('save behavior', function (t) {
  ...
  t.test('to package.json and npm-shrinkwrap.json w/ abs', function (t) {
    common.npm(['install', '--save', 'file:' + testdir + '/pkga'], {cwd: sbdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      ...
      var sdep = shrinkwrap.dependencies.pkga || {}
      // this will be invalid as of the npm-shrinkwrap spec
      t.is(sdep.resolved, 'file:../pkga', 'npm-shrinkwrap.json')
      // this will be required as of the npm shrinkwrap spec
      // t.is(sdep.version, 'file:../pkga', 'npm-shrinkwrap.json')
      t.done()
    })
  })
  ...
  t.test('to package.json and npm-shrinkwrap.json w/ rel', function (t) {
    common.npm(['install', '--save', 'file:../pkgb'], {cwd: sbdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      var pjson = readJson(sbdir + '/package.json')
      var shrinkwrap = readJson(sbdir + '/npm-shrinkwrap.json')
      t.is(pjson.dependencies.pkgb, 'file:../pkgb', 'package.json')
      var sdep = shrinkwrap.dependencies.pkgb || {}
      // this will be invalid as of the npm-shrinkwrap spec
      t.is(sdep.resolved, 'file:../pkgb', 'npm-shrinkwrap.json')
      // this will be required as of the npm shrinkwrap spec
      // t.is(sdep.version, 'file:../pkgb', 'npm-shrinkwrap.json')
      t.done()
    })
  })
  t.test('internal transitive dependencies of shrinkwraps', function (t) {
    common.npm(['install', '--save', 'file:transitive'], {cwd: sbdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      var pjson = readJson(sbdir + '/package.json')
      var shrinkwrap = readJson(sbdir + '/npm-shrinkwrap.json')
      ...
      // this will be invalid as of the npm-shrinkwrap spec
      t.is(sdep.resolved, 'file:transitive', 'npm-shrinkwrap.json direct dep')
      t.is(tdep.resolved, 'file:../pkgc', 'npm-shrinkwrap.json transitive dep')
      // this will be required as of the npm shrinkwrap spec
  ...
  t.test('external transitive dependencies of shrinkwraps', function (t) {
    common.npm(['install', '--save', 'file:../sb-transitive'], {cwd: sbdir, env: conf.env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      var pjson = readJson(sbdir + '/package.json')
      var shrinkwrap = readJson(sbdir + '/npm-shrinkwrap.json')
      t.is(pjson.dependencies['sb-transitive'], 'file:../sb-transitive', 'package.json')
      var sdep = shrinkwrap.dependencies['sb-transitive'] || {}
      var tdep = shrinkwrap.dependencies.sbta
      t.is(tdep, undefined, 'npm-shrinkwrap.json transitive dep')

      // this will be invalid as of the npm-shrinkwrap spec
      t.is(sdep.resolved, 'file:../sb-transitive', 'npm-shrinkwrap.json direct dep')
      // this will be required as of the npm shrinkwrap spec
      // t.is(sdep.version, 'file:../sb-transitive', 'npm-shrinkwrap.json direct dep')
      t.done()
    })
  })
  t.test('external transitive dependencies of shrinkwraps > preserve symlinks', function (t) {
    const env = conf.env.extend({'NODE_PRESERVE_SYMLINKS': '1'})
    common.npm(['install', '--save', 'file:../sb-transitive-preserve'], {cwd: sbdir, env: env}, function (err, code, stdout) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      var pjson = readJson(sbdir + '/package.json')
      var shrinkwrap = readJson(sbdir + '/npm-shrinkwrap.json')
      ...
      // this will be invalid as of the npm-shrinkwrap spec
      t.is(sdep.resolved, 'file:../sb-transitive-preserve', 'npm-shrinkwrap.json direct dep')
      t.is(tdep.resolved, 'file:../sb-transitive-preserve/sbtb', 'npm-shrinkwrap.json transitive dep')
      // this will be required as of the npm shrinkwrap spec
      // t.is(sdep.version, 'file:../sb-transitive-preserve', 'npm-shrinkwrap.json direct dep')
      // t.is(tdep.version, 'file:../sb-transitive-preserve/sbtb', 'npm-shrinkwrap.json transitive dep')
      t.done()
    })
  })
})
